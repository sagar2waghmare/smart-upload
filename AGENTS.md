...
---
## 2026-09-25 — ISSUE 1 + ISSUE 2 root cause fixed (TMDB metadata 502 + metadata request storm)

- **Both reported issues had one shared root cause plus one amplifier; both fixed, committed `cb6fa85` on branch `cloudflare/vinext` (NOT pushed), 2 files, +29/-19.**
- **Root cause A (server — makes ISSUE 1 look like "no details" and ISSUE 2 show the plain title):** `lib/metadata/tmdb.ts` called `fetch(..., { cf: { cacheTtl: 86400, cacheEverything: true } })`. `app/api/metadata/route.ts` is `export const dynamic = "force-dynamic"`, so vinext's patched fetch (`node_modules/vinext/dist/shims/fetch-cache.js:533,805`) injects `cache: "no-store"` into the request. Cloudflare workerd rejects `cacheTtl` combined with no-store and throws `CacheTtl: 86400, is not compatible with cache: no-store header.` — the route catch turned that into HTTP 502 `"TMDB request failed."` for EVERY metadata/episodes call, so details never got rating/runtime/genres/overview and the player never got `meta.logo` (fell back to the plain `.premium-player-state__title` text). The configured production `TMDB_API_KEY` is VALID (a worker probe proved the Worker→TMDB path and the key both fine: fix build returns `200 matched:true` with full meta).
- **Root cause B (client amplifier — request storm):** `lib/use-tmdb-metadata.ts` created a new `embedded` object every render (effect dep) and its guard `cache.has(key) && Boolean(cache.get(key)?.logo)` was never satisfied when the lookup returned null → infinite refetch loop (measured **739 `/api/metadata` requests in ~20 s**, all 502). Results were also lost on overlay unmount (cache only written while `alive`), forcing refetches on reopen.
- **Fix `lib/metadata/tmdb.ts`:** removed both `cf: { cacheTtl/cacheEverything }` options (plain fetch; the shim's no-store path is what Drive fetches already use and work); kept the v3→Bearer fallback; added one server-side `console.error` with upstream status + path on failure (no key, response body unchanged — no client leak).
- **Fix `lib/use-tmdb-metadata.ts`:** `useMemo(itemMeta, [item])` to stabilize `embedded`; guard is now `cache.has(key) || inflight.has(key)` (any cached result, including null, is terminal); `inflight` Set dedupes concurrent details+player mounts; result is always published to the shared cache even if the mount unmounted first (`setState` stays behind `alive`); failed lookup keeps the embedded item metadata instead of wiping it to null.
- **Verification (isolated preview version `8a65c323…`, alias `tmdbfix2`, same Worker code + production secrets; production traffic untouched):** curl `/api/metadata?tmdbId=198184&type=movie` → **200 matched:true** (rating 6.8, runtime 120, genres Crime/Action/Science Fiction, `logo: https://image.tmdb.org/t/p/original/f9jEQr6YSatPrULo6gBpXDiHKdJ.png`); `/api/metadata/episodes?tmdbId=1399&season=1` → 200; logo image → HTTP 200 (220 KB). Mobile CDP run (Pixel-7 profile, real touches): poster tap → details shows `TMDB 6.8 · YEAR 2015 · DURATION 2h 0m` + genres + 385-char overview, close → reopen → Play → **player shows the TMDB CHAPPIE title artwork** (screenshot `shots/fixv2-2-player.png`), no title fallback, browse-page details same content, dashboard scroll 0→600 OK, 0 page errors, **`/api/metadata`: 2 requests, both 200** (vs 739×502). Verdict: ALL CHECKS PASSED. `npm run lint` → 81 errors / 11076 warnings (baseline 83 — no new issues); `npm run build:vinext` → complete.
- **Not done (needs approval):** (1) the fix is only on the preview version — production still runs the broken build until `wrangler deploy` / `wrangler versions deploy`; (2) branch `cloudflare/vinext` (HEAD `cb6fa85`) is not pushed to GitHub. Temp probe worker `tmdb-probe-sagar` deleted, `dist/server/wrangler.probe.json` deleted, 5 `debug-session-*` Firebase users deleted; wrangler has no `versions delete`, so temp preview aliases (`tmdbg`, `tmdbg2`, `tmdbfix`, `tmdbfix2`) linger until removed in the dashboard — they serve non-production versions only.
